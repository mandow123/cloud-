...
CREATE INDEX IF NOT EXISTS admin_password_sessions_account_idx ON admin_password_sessions(account_id, revoked_at, absolute_expires_at);
...
CREATE INDEX IF NOT EXISTS admin_password_attempts_rate_idx ON admin_password_login_attempts(username_hash, request_fingerprint, occurred_at DESC);
...
CREATE TABLE IF NOT EXISTS card_hour_referral_codes (organization_id TEXT PRIMARY KEY, code TEXT NOT NULL UNIQUE, created_at TEXT NOT NULL);
...
INSERT OR IGNORE INTO card_hour_schema_migrations(version,applied_at) VALUES(1,datetime('now'));
INSERT OR IGNORE INTO admin_identity_schema_migrations(version,applied_at) VALUES(4,datetime('now'));
