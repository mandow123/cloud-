...
假设客户实付 100 元，合同约定平台服务费总额 6 元，其中向直接代理分配佣金 3 元；供应商收入为 94 元：
...
供应商开通算力 → 客户使用 → 平台计量 → 客户验收
        ↓
供应商结算 94 元 + 平台净收入 3 元 + 平台服务费内的直接代理佣金 3 元
```

这只是一个便于理解的分账示例，不是已生效价格。记订单总额为 G、平台服务费为 F、费内佣金为 C、供应商收入为 S、平台净收入为 P，则 `0 ≤ C ≤ F`、`S = G − F`、`P = F − C`、`G = S + C + P`。佣金不在平台服务费之外向供应商重复扣减，也不向买家另加价。真实比例、税费、通道费、发票和结算周期必须由合同和支付产品确定。没有代理的订单不会凭空产生佣金。
...
假设订单实付 CNY 100.00，合同约定供应商 94 元、平台服务费总额 6 元，并从该服务费内分配直接代理佣金 3 元，平台净收入 3 元：
