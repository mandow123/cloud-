...
export function assertHostingV2SchemaCompatible(version: number | null | undefined, compatibleThrough = HOSTING_V2_SCHEMA_COMPATIBILITY_VERSION) {
  ...
  const observed = Number(version);
  if (!Number.isSafeInteger(observed) || observed > compatibleThrough) throw new Error("HOSTING_V2_SCHEMA_TOO_NEW");
}
...
export const hostingV2SchemaStatements = [
  ...
  `CREATE INDEX IF NOT EXISTS hosting_v2_device_retirements_org_status_idx ON hosting_v2_device_retirements(organization_id,status,requested_at DESC)`,
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_device_retirement_identity_immutable BEFORE UPDATE ON hosting_v2_device_retirements
  ...
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_device_retirement_status_guard BEFORE UPDATE ON hosting_v2_device_retirements
    WHEN NOT (
  ...
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_device_retirement_immutable_delete BEFORE DELETE ON hosting_v2_device_retirements BEGIN SELECT RAISE(ABORT, 'hosting device retirement immutable'); END`,
  ...
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_fee_tier_immutable_update BEFORE UPDATE ON hosting_v2_fee_tiers BEGIN SELECT RAISE(ABORT, 'hosting fee tier immutable'); END`,
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_fee_tier_immutable_delete BEFORE DELETE ON hosting_v2_fee_tiers BEGIN SELECT RAISE(ABORT, 'hosting fee tier immutable'); END`,
  `INSERT OR IGNORE INTO hosting_v2_fee_tiers(fee_schedule_id,tier_code,minimum_qualifying_micros,platform_fee_bps,referral_reward_bps,created_at)
  ...
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_lifetime_fee_tier_immutable_update BEFORE UPDATE ON hosting_v2_lifetime_fee_tiers BEGIN SELECT RAISE(ABORT, 'hosting lifetime fee tier immutable'); END`,
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_lifetime_fee_tier_immutable_delete BEFORE DELETE ON hosting_v2_lifetime_fee_tiers BEGIN SELECT RAISE(ABORT, 'hosting lifetime fee tier immutable'); END`,
  `INSERT OR IGNORE INTO hosting_v2_lifetime_fee_tiers(fee_schedule_id,tier_code,minimum_qualifying_micros,platform_fee_bps,referral_reward_bps,created_at)
    SELECT id,'STARTER',0,100,CASE WHEN platform_fee_bps=0 THEN 0 ELSE CAST(100*referral_reward_bps/platform_fee_bps AS INTEGER) END,created_at FROM hosting_v2_fee_schedules
  ...
  `CREATE INDEX IF NOT EXISTS hosting_v2_gateway_bindings_device_status_idx ON hosting_v2_gateway_bindings(device_id,status,updated_at DESC)`,
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_gateway_binding_identity_immutable BEFORE UPDATE ON hosting_v2_gateway_bindings
  ...
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_gateway_binding_status_forward_only BEFORE UPDATE OF status ON hosting_v2_gateway_bindings
    WHEN NOT (
  ...
  `CREATE TRIGGER IF NOT EXISTS hosting_v2_gateway_binding_immutable_delete BEFORE DELETE ON hosting_v2_gateway_bindings BEGIN SELECT RAISE(ABORT, 'hosting gateway binding immutable'); END`,
