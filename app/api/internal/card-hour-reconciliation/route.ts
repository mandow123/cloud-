...
import { alipayReconciliationReadiness } from "@/lib/server/alipay-live";
...
export async function POST(request: Request) {
  ...
  if (!qixiangPayReconciliationReadiness().canReconcilePayment
    && !alipayReconciliationReadiness().canReconcilePayment) return Response.json({ error: "RECONCILIATION_DISABLED" }, { status: 503 });
