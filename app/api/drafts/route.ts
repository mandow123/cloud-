...
export async function GET(request: Request) {
  ...
  try {
    ...
    const query = readPageQuery(request, ["mine"] as const, "mine");
    const page = await authorization.store.listOwnedDrafts(actor.id, query);
    return jsonResponse({
      items: page.items,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await authorization.store.consumeWriteAllowance(actor.id, "drafts");
    const idempotencyKey = requireIdempotencyKey(request);
    const input = parseCreateDraft(await readJsonBody(request));
    const result = await authorization.store.createDraft({
      actorId: actor.id,
      idempotencyKey,
