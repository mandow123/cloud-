...
export async function GET(request: Request, contextValue: { params: Promise<{ offerId: string }> }) {
  ...
  try {
    ...
    const [offer, transaction] = await Promise.all([
      store.getPublicOffer(offerId, new Date().toISOString()),
      readHostingV2TransactionAvailability(),
    ]);
    if (!offer) throw new ExchangeDomainError("EXCHANGE_NOT_FOUND", 404, "GPU 报价不存在或已不可成交。");
    return jsonResponse({ record: hostingPublicOfferClientView(offer), updatedAt: new Date().toISOString(), transaction }, 200, undefined, context);
  } catch (error) {
