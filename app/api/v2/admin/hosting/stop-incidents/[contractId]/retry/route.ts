...
export async function POST(request: Request, contextValue: { params: Promise<{ contractId: string }> }) {
  ...
  try {
    ...
    if (!/^hctr_[a-z0-9_-]{8,80}$/u.test(contractId)) throw new AccountAuthError("HOSTING_VALIDATION_ERROR", 400, "停机恢复合同编号无效。 ");
    const input = {
    ...
    const result = await (await getHostingV2Store()).retryFailedStop(contractId, input, mutation);
