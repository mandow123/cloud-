import { parseTestRecordSettlement } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request, contextValue: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = parseTestRecordSettlement(await readJsonBody(request));
    const store = await getExchangeStore();
    if (!store.testRecordSettlement) {
      throw new ExchangeDomainError("EXCHANGE_STATE_CONFLICT", 409, "测试结算台账尚未启用。");
    }
    const result = await store.testRecordSettlement(id, {
      actorId: actor.id,
      idempotencyKey: requireIdempotencyKey(request),
      payloadHash: await mutationHash({ settlementId: id, ...input }),
    }, input);
