import { parseCreatePool } from "@/lib/server/supply-domain";
...
import { requireKaiSelfPresetOperator, supplyWorkspaceRole } from "@/lib/server/supply-api";
...
export async function POST(request: Request) {
  const context = beginApiRequest(request); let actor: MarketplaceActor | undefined;
  try {
    await requireKaiSelfPresetOperator(request);
    const authorization = await authorizeMarketplaceRequest(request); actor = authorization.actor; prepareWrite(request, actor); await persistMarketplaceSession(authorization);
    const input = parseCreatePool(await readJsonBody(request));
    const result = await (await getSupplyStore()).createPool({ actorId: actor.id, idempotencyKey: requireIdempotencyKey(request), payloadHash: await mutationHash(input) }, input);
