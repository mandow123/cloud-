import { parseTestDeliveryConnection } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request, contextValue: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = parseTestDeliveryConnection(await readJsonBody(request));
    const result = await (await getExchangeStore()).testDeliveryConnection(id, {
      actorId: actor.id,
