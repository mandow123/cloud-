import { parseCreateVerificationRun } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request, contextValue: { params: Promise<{ id: string }> }) {
  ...
  try {
    await requireExchangeAdmin(request, ["VERIFICATION_REVIEW"]);
    ...
    const input = parseCreateVerificationRun(await readJsonBody(request));
    const result = await (await getExchangeStore()).createVerification(id, {
      actorId: actor.id,
      idempotencyKey: requireIdempotencyKey(request),
      payloadHash: await mutationHash({ resourceId: id, ...input }),
    }, input);
