...
export async function POST(request: Request, routeContext: { params: Promise<{ approvalId: string }> }) {
  ...
  try {
    ...
    const result = await (await getManagedGpuStore()).rejectApproval(command.context, approvalId, { expectedVersion: managedGpuInteger(command.input, "expectedVersion", 1), actionType });
