import { parseWithdrawCapacityLot } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request, contextValue: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = parseWithdrawCapacityLot(await readJsonBody(request));
    const result = await (await getExchangeStore()).withdrawCapacityLot(id, {
      actorId: actor.id,
      idempotencyKey: requireIdempotencyKey(request),
      payloadHash: await mutationHash({ capacityLotId: id, ...input }),
    }, input);
