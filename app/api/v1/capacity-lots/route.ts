import { parseCreateCapacityLot } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const input = parseCreateCapacityLot(await readJsonBody(request));
    const idempotencyKey = requireIdempotencyKey(request);
    const result = await (await getExchangeStore()).createCapacityLot({
      actorId: actor.id,
      idempotencyKey,
      payloadHash: await mutationHash(input),
    }, input);
    if (account) await bindNewEntityToOrganization({ account, sourceSystem: "EXCHANGE", entityType: "CAPACITY_LOT", entityId: result.record.id, businessIdempotencyKey: idempotencyKey });
