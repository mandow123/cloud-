...
import { alipayReconciliationReadiness, AlipayLiveError, queryVerifiedAlipayTrade } from "@/lib/server/alipay-live";
...
export async function POST(request: Request, contextValue: { params: Promise<{ orderId: string }> }) {
  ...
  try {
    ...
    if (!topup || topup.organizationId !== account.activeOrganization.id || !topup.id || !topup.cardHourMicros || !topup.amountCents
      || (topup.provider !== "QIXIANG_PAY" && topup.provider !== "ALIPAY")) {
      throw new AccountAuthError("CARD_HOUR_TOPUP_RECONCILIATION_INVALID", 409, "付款单不能自动核对，请联系平台处理。 ");
    }
    const reconciliationReady = topup.provider === "QIXIANG_PAY"
      ? qixiangPayReconciliationReadiness().canReconcilePayment
      : alipayReconciliationReadiness().canReconcilePayment;
    ...
    const reconciliationRequest = await store.registerTopupReconciliationRequest({
      ...
      payloadHash: await mutationHash({ action: "RECONCILE_CARD_HOUR_TOPUP", provider: topup.provider, orderId }),
    ...
    try {
      if (topup.provider === "QIXIANG_PAY" && topup.providerPaymentType !== "alipay" && topup.providerPaymentType !== "wxpay") {
        throw new AccountAuthError("CARD_HOUR_TOPUP_RECONCILIATION_INVALID", 409, "付款单不能自动核对，请联系平台处理。 ");
      }
      const event = topup.provider === "QIXIANG_PAY"
        ? await queryQixiangPayOrder({
            orderId: topup.id,
        ...
        : await queryVerifiedAlipayTrade({ orderId: topup.id, amountCents: topup.amountCents });
      if (event.eventType === "PENDING") {
        const headers = new Headers(authorization.actor.responseHeaders);
        headers.set("retry-after", "30");
        return jsonResponse({ record: visible, reconciled: false, replayed: false }, 202, headers, context);
      }
      await store.applyTopupEvent({
        orderId: event.providerOrderId, provider: topup.provider, providerEventId: event.providerEventId,
      ...
      if ((error instanceof QixiangPayError && error.code === "QIXIANG_PAY_OUTCOME_UNKNOWN")
        || (error instanceof AlipayLiveError && error.code === "ALIPAY_REQUEST_FAILED")) {
        const record = await store.getTopupForOrganization(account.activeOrganization.id, orderId);
        if (record) {
          const headers = new Headers(authorization.actor.responseHeaders);
          headers.set("retry-after", "30");
