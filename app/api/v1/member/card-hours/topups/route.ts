...
import { createAlipayCheckoutUrl, alipayReadiness, AlipayLiveError } from "@/lib/server/alipay-live";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = await readJsonBody(request) as { cardHours?: unknown; channel?: unknown; provider?: unknown };
    ...
    if (body.provider !== undefined && body.provider !== "QIXIANG_PAY" && body.provider !== "ALIPAY") throw new AccountAuthError("CARD_HOUR_TOPUP_PROVIDER_INVALID", 400, "请选择已开放的支付服务。 ");
    const channel = body.channel;
    const provider = body.provider ?? "QIXIANG_PAY";
    const readiness = provider === "QIXIANG_PAY" ? qixiangPayReadiness() : alipayReadiness();
    const pilot = provider === "QIXIANG_PAY"
      ? qixiangPayPilotAccess(account.activeOrganization.id)
      : alipayPilotAccess(account.activeOrganization.id);
    ...
    if (!readiness.canCreatePayment || (provider === "QIXIANG_PAY" && !readiness.channels.includes(channel))) {
      if (provider === "QIXIANG_PAY") throw new QixiangPayError("QIXIANG_PAY_NOT_CONFIGURED", readiness.enabled ? "所选充值渠道尚未就绪。" : "人民币充值通道当前保持关闭。");
      throw new AlipayLiveError("ALIPAY_NOT_CONFIGURED", readiness.enabled ? "支付宝直连尚未就绪。" : "支付宝直连当前保持关闭。");
    }
    if (!readiness.merchantAccountRef) {
      if (provider === "QIXIANG_PAY") throw new QixiangPayError("QIXIANG_PAY_NOT_CONFIGURED", "充值商户配置未完成。");
      throw new AlipayLiveError("ALIPAY_NOT_CONFIGURED", "支付宝收款主体配置未完成。");
    }
    ...
    const result = await store.createTopup({
      account, cardHourMicros, amountCents, provider,
      providerMerchantRef: provider === "QIXIANG_PAY" ? readiness.merchantAccountRef : null,
      providerPaymentType: provider === "QIXIANG_PAY" ? paymentType(channel) : null,
      idempotencyKey,
      payloadHash: await mutationHash({ provider, channel, cardHourMicros, amountCents }),
    ...
    const qixiangCheckoutInput: QixiangCheckoutOrder | null = provider === "QIXIANG_PAY" ? {
      ...commonCheckoutInput,
      channel,
      clientIp: trustedQixiangClientIp(request),
    } : null;
    if (qixiangCheckoutInput) validateQixiangPayCheckout(qixiangCheckoutInput);
    ...
    if (!checkoutUrl) {
      let checkout;
      try {
        checkout = qixiangCheckoutInput
          ? await createQixiangPayCheckout(qixiangCheckoutInput)
          : createAlipayCheckoutUrl(commonCheckoutInput);
      } catch (error) {
    ...
    return jsonResponse({
      record: responseRecord, checkoutUrl, provider, channel,
    ...
    if (error instanceof AlipayLiveError) return jsonResponse({ error: { code: error.code, message: error.message, requestId: context.requestId } }, error.code === "ALIPAY_NOT_CONFIGURED" || error.code === "ALIPAY_REQUEST_FAILED" ? 503 : 400, undefined, context);
