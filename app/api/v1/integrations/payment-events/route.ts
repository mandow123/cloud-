import { parsePaymentEvent } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await requireExchangeAdmin(request, ["PAYMENT_OPERATE"]);
    ...
    const input = parsePaymentEvent(await readJsonBody(request));
    const result = await (await getExchangeStore()).applyPaymentEvent({
      actorId: actor.id,
