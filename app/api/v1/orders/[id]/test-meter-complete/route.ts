import { parseTestMeterComplete } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request, contextValue: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = parseTestMeterComplete(await readJsonBody(request));
    const store = await getExchangeStore();
    if (!store.testCompleteMetering) {
      throw new ExchangeDomainError("EXCHANGE_STATE_CONFLICT", 409, "测试计量完结尚未启用。");
    }
    const result = await store.testCompleteMetering(id, {
      actorId: actor.id,
