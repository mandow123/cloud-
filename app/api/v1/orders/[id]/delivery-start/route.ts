import { parseStartProvisioning } from "@/lib/exchange";
import {
  apiErrorResponse,
  beginApiRequest,
  jsonResponse,
...
export async function POST(request: Request, contextValue: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = parseStartProvisioning(await readJsonBody(request));
    const result = await (await getExchangeStore()).startProvisioning(id, {
      actorId: actor.id,
