import { parseTransitionSwapQuote } from "@/lib/exchange";
import {
...
export async function POST(request: Request, contextValue: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = parseTransitionSwapQuote(await readJsonBody(request));
    const result = await (await getExchangeStore()).transitionSwapQuote(id, {
      actorId: actor.id,
      idempotencyKey: requireIdempotencyKey(request),
      payloadHash: await mutationHash({ quoteId: id, ...input }),
    }, input);
