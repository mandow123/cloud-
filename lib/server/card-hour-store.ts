...
export type CardHourTopupRefundStatus = "PENDING" | "APPROVED" | "PROCESSING" | "MANUAL_REQUIRED" | "SUCCEEDED" | "FAILED" | "REJECTED";
export type CardHourTopupRefundRecord = Readonly<{
  id: string;
  topupOrderId: string;
  organizationId: string;
  provider: CardHourTopupProvider;
  amountCents: number;
  cardHourMicros: number;
  status: CardHourTopupRefundStatus;
  requestedBy: string;
  approvedBy: string | null;
  requestReason: string;
  decisionReason: string | null;
  providerRefundRequestId: string;
  providerTransactionId: string | null;
  attemptCount: number;
  errorCode: string | null;
  errorMessage: string | null;
  manualEvidenceDigest: string | null;
  version: number;
  createdAt: string;
  updatedAt: string;
}>;
...
export interface CardHourStore {
  ...
  listDueTopupReconciliations(input: { now: string; staleBefore: string; limit: number }): Promise<Array<{ orderId: string; organizationId: string; attemptCount: number }>>;
  escalateTopupReconciliation(input: { orderId: string; organizationId: string; now: string }): Promise<void>;
  claimTopupReconciliation(input: { organizationId: string; orderId: string; now: string; staleBefore: string; nextEligibleAt: string }): Promise<{ claimed: boolean; claimToken: string | null; attemptCount: number; record: Record<string, unknown> }>;
  ...
  requestTopupRefund(input: { orderId: string; requestedBy: string; reason: string; payloadHash: string; now: string }): Promise<{ record: CardHourTopupRefundRecord; replayed: boolean }>;
  decideTopupRefund(input: { refundId: string; decision: "APPROVE" | "REJECT"; approvedBy: string; reason: string; expectedVersion: number; now: string }): Promise<CardHourTopupRefundRecord>;
  beginTopupRefund(input: { refundId: string; actorId: string; now: string; staleBefore: string }): Promise<{ record: CardHourTopupRefundRecord; claimed: boolean; claimToken: string | null }>;
  finishTopupRefund(input: { refundId: string; claimToken: string; status: "SUCCEEDED" | "FAILED" | "MANUAL_REQUIRED"; providerTransactionId?: string | null; errorCode?: string | null; errorMessage?: string | null; now: string }): Promise<CardHourTopupRefundRecord>;
  confirmManualTopupRefund(input: { refundId: string; approvedBy: string; providerTransactionId: string; evidenceDigest: string; now: string }): Promise<CardHourTopupRefundRecord>;
  getTopupRefund(refundId: string): Promise<CardHourTopupRefundRecord | null>;
  listTopupRefunds(status?: CardHourTopupRefundStatus): Promise<readonly CardHourTopupRefundRecord[]>;
  ...
  settleSupplyOrder(input: import("./supply-card-hour-settlement.ts").SupplyCardHourInput): Promise<import("./supply-card-hour-settlement.ts").SupplyCardHourResult>;
