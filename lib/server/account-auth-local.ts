...
export async function createLocalTestAccountSession(request:Request,options:{store?:AccountAuthStore;env?:Env;now?:Date}={}){
  ...
  const subject=env.KAI_ADMIN_LOCAL_SUBJECT?.trim()||"local-admin";
  const identity=await store.resolveOrCreateIdentity({provider:"LOCAL",tenantKey:"LOCAL",subject,displayName:env.KAI_ADMIN_LOCAL_DISPLAY_NAME?.trim()||"Local Admin",normalizedEmail:null,organizationExternalKey:"LOCAL:KAI",organizationName:"KAI Local Development",verifiedAt:now.toISOString()});
