...
export const adminSectionConfigs: Record<AdminSectionKey, AdminSectionConfig> = {
  ...
  "supply-offers": {
    ...
    fields: [
      ...
      { label: "提交用户", paths: ["facts.accountDisplayName", "facts.accountPrimaryEmail", "supplierActorId", "actorIds"] },
      { label: "主体名称", paths: ["facts.organizationName"] },
      { label: "组织", paths: ["ownership.organizationId"], format: "id" },
      { label: "账号", paths: ["ownership.accountId"], format: "id" },
      ...
      { label: "规格", paths: ["facts.specification"] },
      { label: "交付方式", paths: ["facts.deliveryForm"] },
      { label: "备注", paths: ["facts.notes"] },
