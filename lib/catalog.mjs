/**
 * KAI Cloud deterministic demo catalog.
 *
 ...
 * Every supplier, listing and quote below is fictional demonstration data.
 */

export const DEMO_DISCLAIMER =
  "演示参考价 · 数据与供应商均为虚构，不构成实时成交价或交易承诺";
...
export const suppliers = Object.freeze([
  Object.freeze({
    ...
    name: "星澜智算（演示）",
    shortName: "星澜智算",
    description: "多地域 GPU 裸金属资源演示供应商。",
  ...
  Object.freeze({
    ...
    name: "南湾模型服务（演示）",
    shortName: "南湾模型",
    description: "模型实例与 Token API 演示供应商。",
...
function createListing(definition) {
  ...
  if (!supplier) {
    throw new Error(`Unknown demo supplier: ${definition.supplierId}`);
  }
  return Object.freeze({
    ...definition,
...
export const resourceListings = Object.freeze([
  ...
  createListing({
    ...
    pricingUnit: "百万 Token",
    region: "北京",
    supplierId: "demo-nanwan",
    deliveryForm: "API 服务",
    summary: "兼容常用接口格式的文本推理演示服务。",
    specs: { 模型: "演示推理模型 671B MoE", 上下文: "64K", 并发: "可预留" },
    capacity: "演示日容量 90 亿 Token",
    sla: "演示 SLA 99.9%",
    deliveryLeadTime: "确认后 15 分钟",
    tags: ["Token", "API", "文本生成"],
    featured: true,
    quote: { rangeMin: 3.8, rangeMax: 6.2, median: 4.9, taxIncluded: true, energyIncluded: true, networkIncluded: true, scopeNote: "输入与输出 Token 综合演示价，含税及标准公网。", sampleCount: 42 },
  }),
  createListing({
    id: "token-qwen-sh",
    ...
    supplierId: "demo-juxin",
    deliveryForm: "API 服务",
    summary: "面向中文业务的高并发文本推理演示服务。",
    specs: { 模型: "演示中文 72B", 上下文: "32K", 限流: "600 RPM 起" },
    capacity: "演示日容量 45 亿 Token",
    sla: "演示 SLA 99.95%",
    deliveryLeadTime: "确认后 15 分钟",
    tags: ["Token", "中文", "API"],
    featured: true,
    quote: { rangeMin: 4.6, rangeMax: 7.4, median: 5.8, taxIncluded: true, energyIncluded: true, networkIncluded: true, scopeNote: "输入输出综合演示价，含税、电费和标准公网。", sampleCount: 36 },
  }),
  createListing({
  ...
  createListing({
    ...
    specs: { 维度: "1,024", 单批: "2,048 条", P95: "演示值 120ms" },
    capacity: "演示日容量 120 亿 Token",
    sla: "演示 SLA 99.9%",
    deliveryLeadTime: "即时开通演示",
  ...
  createListing({
    ...
    specs: { 模型: "演示视觉语言 32B", GPU: "2×L40S", 并发: "32 路" },
    capacity: "演示可用 40 个实例",
    sla: "演示 SLA 99.5%",
  ...
  createListing({
    ...
    specs: { 容量单元: "1,000 Token/s", 模型: "演示 70B", 调度: "专属队列" },
    capacity: "演示可用 80 个容量单元",
    sla: "演示 SLA 99.9%",
  ...
  createListing({
    ...
    pricingUnit: "机柜月",
    region: "北京",
    supplierId: "demo-qianfan",
    deliveryForm: "整机柜",
    summary: "双路供电与基础网络接入的标准整机柜。",
    specs: { 功率: "20kW", 机位: "42U", PUE: "演示值 ≤1.35" },
    capacity: "演示可用 18 柜",
    sla: "演示 SLA 99.99%",
  ...
  createListing({
    ...
    specs: { 功率: "30kW", 冷却: "风液混合", PUE: "演示值 ≤1.30" },
    capacity: "演示可用 420kW",
    sla: "演示 SLA 99.99%",
  ...
  createListing({
    ...
    specs: { 功率: "25kW 起", 冷却: "冷板液冷", PUE: "演示值 ≤1.22" },
    capacity: "演示可用 600kW",
    sla: "演示 SLA 99.9%",
    deliveryLeadTime: "确认后 7 个工作日",
    tags: ["液冷", "绿色算力", "按 kW"],
  ...
  createListing({
    ...
    supplierId: "demo-xinglan",
    deliveryForm: "云主机",
    summary: "按小时弹性的 H100 8 卡云主机演示资源。",
...
const marketDefinitions = Object.freeze([
  ...
  { id: "market-token-million", category: "token_model", label: "KAI 模型调用成本指数（演示历史）", pricingUnit: "百万 Token", region: "全国六大节点", base: 5.3, seed: 7, samples: 91 },
...
export function normalizeQuote(listingOrQuote, context = {}) {
  ...
  if (!quote || quote.currency !== "CNY" || !PRICING_UNITS.includes(quote.pricingUnit)) {
    throw new TypeError("normalizeQuote requires a valid CNY demo quote");
  }
  return Object.freeze({
    sourceListingId: context.sourceListingId ?? listing?.id ?? null,
    supplierId: context.supplierId ?? listing?.supplierId ?? null,
    supplierName: context.supplierName ?? listing?.supplierName ?? "演示供应商",
    title: context.title ?? listing?.title ?? "标准化演示报价",
    currency: "CNY",
    ...
    methodology: "按相同区域、交付形态与计价单位标准化；演示结果不代表真实成交。",
    demo: true,
    disclaimer: DEMO_DISCLAIMER,
  });
}
 ...
 * @param {unknown} seed
 */
export function createDemoRequestId(kind, seed = "kai-cloud-demo") {
  if (!DEAL_MODES.includes(kind)) {
    throw new TypeError(`Unsupported demo request kind: ${kind}`);
  }
  const prefix = kind === "swap" ? "SWP" : kind === "service" ? "SRV" : "RNT";
  return `KAI-${prefix}-DEMO-${fnv1a(`${kind}:${stableSerialize(seed)}`)}`;
}

export const demoRentalRequests = Object.freeze([
  Object.freeze({
    id: createDemoRequestId("rental", "gpu-h100-beijing-64"),
    requestType: "rental",
    dealMode: "rental",
  ...
  Object.freeze({
    id: createDemoRequestId("service", "token-api-shanghai-2000"),
    requestType: "rental",
    dealMode: "service",
...
export const demoSwapRequests = Object.freeze([
  Object.freeze({
    id: createDemoRequestId("swap", "a800-for-h20-demo"),
    requestType: "swap",
...
export const demoNormalizedQuotes = Object.freeze(
