import { supplierReferenceListings } from "../data/gpu-supplier-reference-catalog.mjs";
import { shanghaiHonghuanGpuListings, shanghaiHonghuanSupplier } from "../data/shanghai-honghuan-gpu-catalog.mjs";
import { applySupplierListingMarkup } from "./pricing-policy.mjs";
...
export const regions = Object.freeze([
  ...
  Object.freeze({ id: "nationwide", name: "全国", hub: "多地域", latencyNote: "具体机房见资源详情" }),
  Object.freeze({ id: "overseas", name: "海外", hub: "境外", latencyNote: "跨境条件需重新确认" }),
]);
...
function createQuote({
  ...
  return Object.freeze({
    currency: "CNY",
    pricingUnit,
    rangeMin: applySupplierListingMarkup(rangeMin),
    rangeMax: applySupplierListingMarkup(rangeMax),
    median: applySupplierListingMarkup(median),
    taxIncluded,
    energyIncluded,
    networkIncluded,
    scopeNote: `${scopeNote} 平台挂牌价已按统一策略上调 50%。`,
    sampleCount,
    validUntil,
    updatedAt,
...
export const resourceListings = Object.freeze([
  ...
  createListing({
    ...
    specs: { GPU: "8×A800 80GB", 互联: "NVLink", 最小批次: "2,000.00 卡时" },
    capacity: "平台初始化样本：48,000.00 卡时，供应方接入后核验",
    sla: "人工撮合，以双方确认为准",
  ...
  createListing({
    ...
    specs: { GPU: "H20 96GB", 形态: "8 卡节点", 最小批次: "5,000.00 卡时" },
    capacity: "平台初始化样本：120,000.00 卡时，供应方接入后核验",
    sla: "人工撮合，以双方确认为准",
  ...
  ...supplierReferenceListings,
  ...shanghaiHonghuanGpuListings,
]);
...
export function formatCardHourQuote(value, pricingUnit) {
  const amount = Number(value);
  if (!Number.isFinite(amount)) return "—";
  const basis = {
    "卡时": "GPU 小时",
  ...
  return `${(amount / 1.002).toFixed(2)} KAI 标准卡时 / ${basis}`;
}
