...
export const resourceListings = Object.freeze([
  createListing({
    ...
    capacity: "当前目录容量：12 台 / 96 卡，待供应方核验",
  ...
  createListing({
    ...
    capacity: "当前目录容量：20 台 / 160 卡，待供应方核验",
    sla: "目标 SLA 99.9%，供应方接入后确认",
  ...
  createListing({
    ...
    capacity: "当前目录容量：48,000 卡时，待供应方核验",
    sla: "人工撮合，以双方确认为准",
  ...
  createListing({
    ...
    capacity: "当前目录容量：18 台 / 144 卡，待供应方核验",
  ...
  createListing({
    ...
    capacity: "当前目录容量：120,000 卡时，待供应方核验",
    sla: "人工撮合，以双方确认为准",
  ...
  createListing({
    ...
    capacity: "当前目录容量：日容量 90 亿 Token，待供应方核验",
    sla: "目标 SLA 99.9%，供应方接入后确认",
  ...
  createListing({
    ...
    capacity: "当前目录容量：日容量 45 亿 Token，待供应方核验",
    sla: "目标 SLA 99.95%，供应方接入后确认",
    deliveryLeadTime: "确认后 15 分钟",
    tags: ["Token", "中文", "API"],
    featured: true,
  ...
  createListing({
    ...
    capacity: "当前目录容量：日容量 120 亿 Token，待供应方核验",
    sla: "目标 SLA 99.9%，供应方接入后确认",
  ...
  createListing({
    ...
    capacity: "当前目录容量：80 个容量单元，待供应方核验",
    sla: "目标 SLA 99.9%，供应方接入后确认",
  ...
  createListing({
    ...
    capacity: "当前目录容量：240,000 kW·h，待供应方核验",
    sla: "人工撮合，以双方确认为准",
  ...
  createListing({
    ...
    capacity: "当前目录容量：36,000 容量时，待供应方核验",
    sla: "人工撮合，以双方确认为准",
    deliveryLeadTime: "意向确认后 3 个工作日",
    tags: ["容量置换", "训练", "专属集群"],
    featured: false,
  ...
  createListing({
    ...
    capacity: "当前目录容量：峰值 320 卡，待供应方核验",
    sla: "目标 SLA 99.9%，供应方接入后确认",
  ...
  createListing({
    ...
    capacity: "当前目录容量：30 个容量单元，待供应方核验",
    sla: "目标 SLA 99.95%，供应方接入后确认",
    deliveryLeadTime: "确认后 1 个工作日",
    tags: ["预留容量", "H800", "集群"],
    featured: false,
  ...
  createListing({
    ...
    capacity: "当前目录容量：48 个容量单元，待供应方核验",
    sla: "目标 SLA 99.9%，供应方接入后确认",
  ...
  createListing({
    ...
    capacity: "当前目录容量：18,000 服务器时，待供应方核验",
    sla: "人工撮合，以双方确认为准",
    deliveryLeadTime: "意向确认后 3 个工作日",
    tags: ["云算力", "置换", "跨区域"],
    featured: true,
...
function createMarketSeries(definition) {
  const seriesDays = 720;
  const preservedSuffixDays = 90;
  const end = Date.parse(`${UPDATED_AT.slice(0, 10)}T00:00:00.000Z`);
  const start = end - (seriesDays - 1) * 86_400_000;
  const points = Array.from({ length: seriesDays }, (_, index) => {
    // Keep the previously published 90-day suffix byte-for-byte stable while
    // extending the reviewed historical sample backwards to 720 days.
    const historicalIndex = index - (seriesDays - preservedSuffixDays);
    const cycle = Math.sin((historicalIndex + definition.seed) * 0.23) * 0.035;
    const modulo = ((historicalIndex * definition.seed) % 13 + 13) % 13;
    const sampleModulo = ((historicalIndex * definition.seed) % 17 + 17) % 17;
    const spreadModulo = ((historicalIndex + definition.seed) % 5 + 5) % 5;
    const micro = (modulo - 6) * 0.0017;
    const trend = (historicalIndex - 44.5) * 0.00028;
    const p50 = roundMarketValue(definition.base * (1 + cycle + micro + trend));
    const spread = 0.085 + spreadModulo * 0.006;
    ...
    return Object.freeze({
      ...
      p50,
      p75,
      sampleCount: definition.samples + sampleModulo - 8,
    });
  });
