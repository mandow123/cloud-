...
import { filterAndSortResources, parseResourceQuery } from "@/lib/market";
import { summarizeResourceCatalog } from "@/lib/resource-freshness";
...
function directoryRequestHref(resource: ResourceListing) {
...
export function ResourceExplorer({ listings }: { listings: readonly ResourceListing[] }) {
  ...
  const catalogSummary = useMemo(() => summarizeResourceCatalog(listings), [listings]);
  ...
  return (
    <div>
      <section className="border-b border-[var(--border)] bg-[var(--surface)]">
        <div className="shell py-14 sm:py-16">
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
            <div>
              <p className="kicker">REFERENCE RESOURCE DIRECTORY</p>
              <h1 className="m-0 max-w-4xl text-4xl leading-[1.08] text-[var(--ink)] sm:text-5xl">算力资源目录</h1>
              <p className="section-lead">
                按资源类型、区域与交付形态发现历史参考方案，再提交算力需求由平台重新核验供给、价格与交付条件。
              </p>
            </div>
            <div className="border-t-2 border-[var(--accent)] bg-[var(--info-bg)] px-5 py-4">
              <div className="flex items-baseline justify-between gap-6">
                <span className="text-xs font-semibold text-[var(--muted)]">目录参考方案</span>
                <strong className="text-3xl tabular-nums text-[var(--ink)]">{listings.length}</strong>
              </div>
              <p className="mt-2 mb-0 text-xs leading-5 text-[var(--muted)]">{catalogSummary.expired} 条历史报价已过期；这里不展示实时库存或可成交报价。</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="shell py-10 sm:py-12">
        <aside className="market-notice mb-8" aria-label="目录数据声明">
          <p className="m-0"><strong>历史初始化样本：</strong>{catalogSummary.total} 条目录记录均用于资源发现与需求描述，不代表实时库存、现货或可成交报价。</p>
          <p className="m-0 whitespace-nowrap font-semibold text-[var(--warning)]">{catalogSummary.expired} 条报价已过期</p>
        </aside>
        ...
        <div className="grid items-start gap-8 lg:grid-cols-[260px_minmax(0,1fr)]">
          <aside className="border border-[var(--border)] bg-[var(--info-bg)] lg:sticky lg:top-28" aria-label="资源筛选">
            <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-4">
              <h2 className="m-0 text-base text-[var(--ink)]">筛选参考目录</h2>
              {activeFilterCount > 0 && (
                <button
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[var(--border-strong)] pb-4">
              <div>
                <p className="m-0 text-xs font-semibold tracking-wide text-[var(--muted)]">MATCHED REFERENCES</p>
                <p className="mt-1 mb-0 text-lg font-semibold text-[var(--ink)]">
                  找到 <span className="tabular-nums text-[var(--accent)]">{results.length}</span> 项参考方案
                </p>
              </div>
                  ...
                  <table className="w-full min-w-[680px] border-collapse text-sm">
                    ...
                    <tbody>
                      {[
                        ...
                        ["历史口径", (item: ResourceListing) => pricingScope(item)],
                        ...
                        <tr key={label as string} className="border-t border-[var(--border)]">
                          ...
                          {compared.map((resource) => (
                            <td key={resource.id} className="border-r border-[var(--border)] p-3 align-top text-[var(--text)] last:border-r-0">
                              ...
                              {label === "数据状态" && <span className="mt-1 block text-xs text-[var(--warning)]">提交需求后重新核验</span>}
                            </td>
                          ))}
                        </tr>
                      ))}
            ...
            {results.length === 0 ? (
              <div className="mt-6 border-y border-[var(--border)] bg-[var(--surface)] px-6 py-20 text-center">
                <p className="m-0 text-xl font-semibold text-[var(--ink)]">没有匹配的参考方案</p>
                <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-[var(--muted)]">
                  当前筛选组合较窄。清除筛选后可浏览完整参考目录，或提交算力需求由 KAI 进行人工询价。
                </p>
                      ...
                      {results.map((resource) => (
                        <tr key={resource.id}>
                          <td className="min-w-64">
                            ...
                            <span className="mt-1 block text-xs text-[var(--muted)]">{publicCatalogText(resource.supplierName)} · 初始化来源档案</span>
                          </td>
                          ...
                          <td className="min-w-44">
                            <strong className="block text-[var(--warning)]">报价已过期</strong>
                            <span className="mt-1 block text-xs text-[var(--muted)]">历史初始化样本 · 不代表可用库存</span>
                            <span className="mt-1 block text-xs text-[var(--muted)]">有效至 {resource.quote.validUntil}</span>
                          </td>
                          <td className="min-w-32">
                            ...
                            <Link
                              className="button button-primary button-compact mt-2"
                              href={directoryRequestHref(resource)}
                              aria-label={`基于 ${resource.title} 提交算力需求`}
                            >
                              <span>提交需求</span><span aria-hidden="true">→</span>
                            </Link>
                          </td>
                        </tr>
                      ))}
                    ...
                    <article key={resource.id} className="border-t-2 border-[var(--border-strong)] bg-[var(--surface)] p-5 ring-1 ring-[var(--border)]">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 mb-0 text-xs text-[var(--muted)]">{publicCatalogText(resource.supplierName)} · 初始化来源档案</p>
                        </div>
                      ...
                      <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
                        <div>
                          <p className="m-0 text-xs text-[var(--muted)]">数据状态</p>
                          <p className="mt-1 mb-0 text-lg font-semibold text-[var(--warning)]">历史初始化样本 · 报价已过期</p>
                          <p className="mt-1 mb-0 text-xs text-[var(--muted)]">提交需求后重新核验供给、价格与交付条件</p>
                        </div>
            ...
            {results.length > 0 && (
              <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-[var(--border)] pt-4 text-xs text-[var(--muted)]">
                <span>共 {results.length} 项历史参考 · 不代表可用库存</span>
                <span>真实 GPU 可成交报价仅在 /gpu 展示</span>
              </div>
            )}
          </div>
        </div>
