...
export function SupplyOfferCreate() {
  ...
  const [rate, setRate] = useState("3.60");
      ...
      {dashboard ? <div className={styles.formLayout}>
        <form className={styles.formPanel} onSubmit={submit}>
          <div className={styles.fieldGrid}>
            ...
            <label className={styles.field}><span>KAI 标准卡时 / GPU 小时</span><input inputMode="decimal" onChange={(event) => setRate(event.target.value)} pattern="\d{1,9}(\.\d{1,2})?" required value={rate} /><small>卡时价格最多 2 位小数；人民币仍按 1 卡时 = ¥1.002 显示参考。</small></label>
