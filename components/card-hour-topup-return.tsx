...
export function CardHourTopupReturn({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const current = await check(controller.signal);
        if (cancelled || (current.status === "CAPTURED" && current.credited) || current.status === "CLOSED" || current.status === "RECONCILIATION_REQUIRED") {
          setChecking(false);
          return;
        }
        pollCount.current += 1;
  ...
  const detail = credited
    ...
    : closed
      ...
      : reconciliationRequired
        ? "支付结果需要平台人工核对。请勿重复付款，也不要重新发起充值；平台完成核对后会更新本页状态。"
  ...
  return (
    <div className={styles.returnPage}>
      <section className={styles.returnPanel} aria-live="polite">
        ...
        {record ? <dl className={styles.returnFacts}><div><dt>付款单</dt><dd>{record.id}</dd></div><div><dt>支付方式</dt><dd>{channelLabels[record.channel]}</dd></div><div><dt>购买卡时</dt><dd>{formatCardHourDisplayMicros(record.cardHourMicros)}</dd></div><div><dt>支付金额</dt><dd>{money(record.amountCents)}</dd></div><div><dt>服务端状态</dt><dd>{credited ? "已确认并入账" : closed ? "已关闭" : "处理中"}</dd></div></dl> : null}
        {checking && !credited && !closed && !reconciliationRequired ? <p className={styles.notice} role="status">正在定时向平台服务端查询，不读取浏览器回跳参数作为成功依据…</p> : null}
        {reconciliationRequired ? <p className={styles.error} role="alert">待平台人工核对，请勿重复付款或重新发起充值。</p> : null}
        {error ? <p className={styles.error} role="alert">{error}</p> : null}
        {!checking && !credited && !closed && !reconciliationRequired ? <p className={styles.notice}>支付结果仍未确认。延迟并不等于失败，也不会因此自动入账。</p> : null}
        <div className={styles.actions}>
          {!checking && !credited && !closed && !reconciliationRequired ? <button className={styles.primaryAction} onClick={() => { setChecking(true); pollCount.current = 0; void check().then(() => setChecking(false)).catch((reason: unknown) => { setError(reason instanceof Error ? reason.message : "付款单状态暂时无法读取。"); setChecking(false); }); }} type="button">重新查询</button> : null}
