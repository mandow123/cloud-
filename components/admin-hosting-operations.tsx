...
type HostingReadinessItem = Readonly<{
  key: string;
  label: string;
  ready: boolean;
  reason: string;
...
function hostingReadinessFromPayload(payload: Record<string, unknown>): HostingReadiness | null {
  ...
  return {
    ...
    items: [
      item("identity", "统一登录", "用户登录、组织身份与供应商权限的唯一可信来源。", "/login?returnTo=%2Fsupply", "验证登录", identity ? { ready: identity.available === true, reason: typeof identity.errorCode === "string" ? identity.errorCode : identityMissing ?? "KAI_IDENTITY_NOT_READY" } : null),
...
async function hostingOperationsBundle() {
  ...
  const [grants, profiles, fee, cleanupIncidents, stopIncidents, disputes, readiness] = await Promise.all([
    ...
    adminGetReadinessJson().then(hostingReadinessFromPayload),
  ]);
  return { session, grants, profiles, fee, cleanupIncidents, stopIncidents, disputes, readiness };
}
...
export function AdminHostingOperations() {
  ...
  const [readiness, setReadiness] = useState<HostingReadiness | null>(null);
  ...
  return (
    <div className="admin-page">
      ...
      {session && readiness ? <section className="admin-panel admin-hosting-panel admin-hosting-readiness" aria-labelledby="hosting-readiness-title">
        <div className="admin-panel-head"><div><p className="admin-kicker">Backend readiness</p><h2 id="hosting-readiness-title">上线闭环就绪状态</h2></div><span className={`admin-status ${readiness.enabled && readiness.ready ? "success" : "warning"}`}>{readiness.enabled && readiness.ready ? "邀请制交易已就绪" : readiness.configurationEnabled ? "配置阶段" : "交易安全关闭"}</span></div>
        <p className="admin-panel-copy">这里直接读取交易接口使用的同一份服务端判定。任意关键项未通过时，挂牌与租用都会保持关闭，不会出现前端显示可用、后端实际不可用的情况。</p>
        <div className="admin-hosting-readiness-meta"><span>发布模式 <b>{readiness.rolloutMode}</b></span><span>配置接口 <b>{readiness.configurationEnabled ? "已开放" : "关闭"}</b></span><span>交易接口 <b>{readiness.enabled ? "已开放" : "关闭"}</b></span></div>
        <div className="admin-hosting-readiness-list">{readiness.items.map((entry) => <article className={entry.ready ? "pass" : "fail"} key={entry.key}>
          <span className={`admin-status ${entry.ready ? "success" : "danger"}`}>{entry.ready ? "已就绪" : "未就绪"}</span>
          <div><h3>{entry.label}</h3><p>{entry.detail}</p><small>{entry.reason}</small></div>
