...
type Dashboard = {
  ...
  ledger: Array<{ operation: string; business_key: string; side: string; amount_micros: number; balance_after_micros: number; created_at: string }>;
};
...
export function CardHourAccountPanel() {
  ...
  return (
    <section className="mb-16 scroll-mt-28" id="card-hours" aria-labelledby="card-hour-title">
      ...
      <section className="mt-8 bg-[var(--surface)] ring-1 ring-[var(--border)]" id="card-hour-ledger"><div className="border-b border-[var(--border)] p-6"><p className="kicker">LEDGER</p><h3 className="m-0 text-2xl">卡时明细</h3></div>{dashboard.ledger.length ? <div className="overflow-x-auto"><table className="w-full border-collapse text-left text-sm"><thead className="bg-[var(--info-bg)] text-xs text-[var(--muted)]"><tr><th className="p-4">时间</th><th className="p-4">类型</th><th className="p-4">来源</th><th className="p-4 text-right">变动</th><th className="p-4 text-right">余额</th></tr></thead><tbody>{dashboard.ledger.map((item) => <tr className="border-t border-[var(--border)]" key={`${item.business_key}:${item.created_at}`}><td className="p-4 whitespace-nowrap">{time(item.created_at)}</td><td className="p-4">{operationLabels[item.operation] ?? item.operation}</td><td className="p-4 font-mono text-xs">{item.business_key}</td><td className="p-4 text-right font-mono tabular-nums">{item.side === "CREDIT" ? "+" : "−"}{cardHours(item.amount_micros)}</td><td className="p-4 text-right font-mono tabular-nums">{cardHours(item.balance_after_micros)}</td></tr>)}</tbody></table></div> : <p className="m-0 p-6 text-sm text-[var(--muted)]">暂无卡时明细。所有充值、消费、退款和收益都会以不可修改的账本记录展示。</p>}</section>
    </section>
  );
}
