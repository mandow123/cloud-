  ...
  return (
    <section className="mb-16 scroll-mt-28" id="card-hours" aria-labelledby="card-hour-title">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(320px,420px)]">
        ...
        <form className="h-fit bg-[var(--surface)] p-6 ring-1 ring-[var(--border)]" id="topup" onSubmit={buyCardHours}><p className="kicker">BUY CARD HOURS</p><h3 className="m-0 text-2xl">购买卡时</h3><p className="mt-3 text-sm text-[var(--muted)]">为避免人民币分币误差，购买数量必须是 5.00 卡时的整数倍；5.00 卡时正好是 ¥5.01。</p>{!topupReady ? <p className="border-l-4 border-[var(--warning)] bg-[var(--warning-bg)] p-4 text-sm text-[var(--ink)]" role="status"><strong className="block">公开购买暂未开放</strong><span>{dashboard.topupAvailability.reason}</span></p> : null}<label className="mt-5 grid gap-2 text-sm font-semibold text-[var(--ink)]" htmlFor="card-hour-topup">卡时数量<input className="min-h-12 rounded-none border border-[var(--border-strong)] bg-[var(--canvas)] px-4 text-right font-mono tabular-nums" id="card-hour-topup" inputMode="numeric" min="5" onChange={(event) => setTopup(event.target.value)} step="5" type="number" value={topup} /></label><div className="mt-4 grid grid-cols-3 gap-px bg-[var(--border)]">{[100, 500, 1000].map((amount) => <button className="min-h-11 rounded-none border-0 bg-[var(--info-bg)] font-mono text-sm text-[var(--ink)] hover:bg-[var(--accent-soft)]" key={amount} onClick={() => setTopup(String(amount))} type="button">{amount.toFixed(2)}</button>)}</div><dl className="my-5 grid gap-px bg-[var(--border)]"><div className="flex justify-between bg-[var(--info-bg)] p-4"><dt>应付人民币</dt><dd className="m-0 font-mono font-semibold tabular-nums">{validTopup ? `¥${payable.toFixed(2)}` : "—"}</dd></div><div className="flex justify-between bg-[var(--info-bg)] p-4"><dt>到账卡时</dt><dd className="m-0 font-mono font-semibold tabular-nums">{validTopup ? topupNumber.toFixed(2) : "—"}</dd></div></dl><button className="button button-primary w-full rounded-none" disabled={!topupReady || !validTopup || busy !== null} type="submit">{busy === "topup" ? "正在创建付款单…" : topupReady ? "用人民币购买卡时" : "人民币购买尚未开放"}</button><p className="mb-0 mt-3 text-xs text-[var(--muted)]">只有支付服务端验签成功后才会入账；浏览器返回页面不代表购买成功。</p></form>
      </div>
