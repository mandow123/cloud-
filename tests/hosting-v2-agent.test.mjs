...
async function approvedSupplier(store, now) {
  ...
  await store.reviewProfile(account.activeOrganization.id, { decision: "APPROVE", expectedVersion: 2, reviewNote: "内部设备验真测试通过", evidenceDigest: "a".repeat(64) }, mutation("admin-agent-reviewer", "agent-profile-review", "agent-profile-review-hash", now));
}
...
test("signed Host Agent registration, heartbeat and verification close without replay", async () => {
  ...
  try {
    ...
    assert.equal(challenge.minimumAgentVersion, "1.9.6");
    ...
    const registration = { operation: "REGISTER_DEVICE", challengeId: challenge.id, nonce: challenge.nonce, displayName: "4090 工作站 01", devicePublicKey, agentVersion: "1.9.6", inventory, inventoryDigest, issuedAt, expiresAt };
