...
test("SSH provisioning, start and stop remain inside verified device boundaries", async () => {
  ...
  try {
    ...
    const running = await store.completeCommand("had_lifecycle", startCommand.id, { outcome: "SUCCEEDED", evidenceDigest: `sha256:${"7".repeat(64)}`, details: {} }, mutation("lifecycle-start-result", "lifecycle-start-result-hash", now));
