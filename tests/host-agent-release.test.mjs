...
test("Host Agent release is deterministic, checksummed and contains only reviewed source files", async () => {
  ...
  try {
    ...
    assert.equal(first.archive, "kai-host-agent-1.8.0.tgz");
    ...
    for (const entry of entries) {
      assert.match(entry, /^kai-host-agent-1\.8\.0\/[A-Za-z0-9._/-]+$/u);
      assert.doesNotMatch(entry, /(?:^|\/)\.\.?\/|identity\.json|pairing\.json|\.env$/u);
    }
    assert.ok(entries.includes("kai-host-agent-1.8.0/release-manifest.json"));
    assert.ok(entries.includes("kai-host-agent-1.8.0/src/doctor.mjs"));

    execFileSync("tar", ["-xzf", join(outputDirectory, first.archive), "-C", extractionDirectory]);
    const manifest = JSON.parse(await readFile(join(extractionDirectory, "kai-host-agent-1.8.0", "release-manifest.json"), "utf8"));
