...
test("only approved, verified and fee-backed GPU offers enter the public market", async () => {
  ...
  try {
    ...
    const device = await store.registerDevice(challenge.id, { displayName: "4090 报价机", deviceKeyId: `sha256:${"4".repeat(64)}`, devicePublicKey: "A".repeat(43), agentVersion: "1.9.2", inventory, inventoryDigest }, mutation("agent-offer", "offer-device-register", "offer-device-register-hash", now));
    ...
    upgrade.prepare("UPDATE hosting_v2_devices SET agent_version='1.9.2' WHERE id=?").run(device.id);
    upgrade.close();
