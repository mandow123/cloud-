...
test("the production graph excludes the vulnerable image-size runtime", async () => {
  const [packageJson, packageLock, standaloneVinext, standaloneReact, standaloneReactDom, standaloneScheduler, vinextPluginStub, metadataStub, workerImageStub] = await Promise.all([
    ...
    readFile(resolve(projectRoot, "dist/standalone/node_modules/react/package.json"), "utf8").then(JSON.parse),
    readFile(resolve(projectRoot, "dist/standalone/node_modules/react-dom/package.json"), "utf8").then(JSON.parse),
    readFile(resolve(projectRoot, "dist/standalone/node_modules/scheduler/package.json"), "utf8").then(JSON.parse),
  ...
  assert.equal(standaloneReact.version, "19.2.8");
  assert.equal(standaloneReactDom.version, "19.2.8");
  assert.equal(standaloneScheduler.version, "0.27.0");
  ...
  const prodServer = await import(resolve(projectRoot, "dist/standalone/node_modules/vinext/dist/server/prod-server.js"));
  assert.equal(typeof prodServer.startProdServer, "function");
});
