...
test("insufficient model index history remains disclosed while homepage separates catalog prices from real offers", async () => {
  ...
  assert.match(home, /模型价格档位/);
  assert.match(home, /条公开目录价快照/);
  assert.match(home, /模型价格档位不是已核验资源、库存或可成交报价/);
  assert.doesNotMatch(home, /可交易报价|条已核验资源/);
