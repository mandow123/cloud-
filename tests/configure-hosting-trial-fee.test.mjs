...
test("trial fee setup logs in as Root, uses CSRF/idempotency and logs out without returning credentials", async () => {
  ...
  const fetcher = async (url, init = {}) => {
    calls.push({ url, init });
    if (url.endsWith("/api/auth/admin/password")) return new Response(JSON.stringify({ admin: { principal: { roles: ["ROOT"] } } }), { status: 200, headers: { "content-type": "application/json", "set-cookie": "__Host-kai_account=secret-cookie; Path=/; Secure; HttpOnly" } });
    if (url.endsWith("/api/session")) return Response.json({ session: { csrfToken: "csrf-token-long-enough-for-test" } });
